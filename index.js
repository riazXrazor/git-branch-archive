...
// remove empty name
merged_branches = merged_branches.filter(function(name){ return name != ''; });
...
function ask_archive()
{
  ...
  inquirer.prompt([{
    type: 'confirm',
    name: 'a',
    message: chalk.green('Archive branch '+ chalk.red(branch) +' (just hit enter for YES)?'),
    default: true
...
function create_archive_tags()
{
  var branch = archived_branches[counter];
      ...
      inquirer.prompt([{
        type: 'confirm',
        name: 'a',
        message: chalk.green('Push archive tags to remote (just hit enter for YES)?'),
        default: true
      }]).then(function (answers) {
       if (answers.a) {
...
function delete_remote_branches()
{
  var branch = delete_branches[counter];
      ...
      inquirer.prompt([{
        type: 'confirm',
        name: 'a',
        message: chalk.green('Delete '+ chalk.red(branch) +' branch from remote (just hit enter for YES)?'),
        default: true
      }]).then(function (answers) {
       if (answers.a) {
...
function delete_local_branches()
{
  var branch = delete_branches[counter];
      ...
      inquirer.prompt([{
        type: 'confirm',
        name: 'a',
        message: chalk.green('Delete '+ chalk.red(branch) +' branch from local (just hit enter for YES)?'),
        default: true
      }]).then(function (answers) {
       if (answers.a) {
